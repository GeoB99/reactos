    ...
    _In_ POWER_POLICY_DEVICE_TYPE PolicyDeviceType)
{
     ...
     * (the Battery Class driver will do that). There is no need to open the same
    ...
    {
        /* Get a device object of this policy device to gather info of what it is */
        Status = PopGetPolicyDeviceObject(NotificationStructure->SymbolicLinkName,
                                          &PolicyDeviceObject);
        if (!NT_SUCCESS(Status))
        {
    ...
    switch (PolicyDeviceType)
    {
        ...
        case PolicyDeviceBattery:
        {
             ...
             */
            Status = PopAddCompositeBattery(PolicyDeviceObject, NotificationStructure->SymbolicLinkName);
            PolicyData = NULL;
            PolicyWorkerRoutine = PopCompositeBatteryHandler;
            break;
        }
    ...
    _In_ PVOID Context)
{
    ...
    /* Passing a NULL policy notification or policy context is not what we expect here */
    if (!NotificationStructure || !Context)
    {
        DPRINT1("Got NULL policy notification or policy context\n");
